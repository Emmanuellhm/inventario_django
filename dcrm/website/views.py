...
# Usamos ReportLab para la generación de PDFs en Windows (importación diferida)
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        if 'admin' in user_roles or request.user.is_superuser:
...
def dashboard(request):
    """Dashboard principal: si es admin redirige al panel admin, si es usuario muestra el listado de clientes."""
    user_roles = set(request.user.roles.values_list('name', flat=True))
    if 'admin' in user_roles or request.user.is_superuser:
        return redirect('customadmin:dashboard')

    # Para usuarios normales, mostrar el listado de clientes (solo lectura)
    is_admin = False
...
def register_user(request):
    ...
    if request.method == 'POST':
        ...
        # Incluye asterisco (*) como carácter permitido en la contraseña
...
def view_record(request, pk):
    ...
    cliente = get_object_or_404(Cliente, pk=pk)
    user_roles = set(request.user.roles.values_list('name', flat=True))
    is_admin = 'admin' in user_roles or request.user.is_superuser
    context = {
        'cliente': cliente,
        'is_admin': is_admin,
    }
    if is_htmx(request):
        return render(request, 'view_record_partial.html', context)
